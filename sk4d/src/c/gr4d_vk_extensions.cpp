...
bool gr4d_vk_extensions_has_extension(const gr_vk_extensions_t* self, const char name[], uint32_t min_version) {
    return SK4D_ONLY_VULKAN(AsGrVkExtensions(self)->hasExtension(name, min_version), false);
}

void gr4d_vk_extensions_init(gr_vk_extensions_t* self, void* context, gr_vk_get_proc proc, gr_vk_instance_t instance, gr_vk_physicaldevice_t physical_device, const char* instance_extensions[], int32_t instance_extension_count, const char* device_extensions[], int32_t device_extension_count) {
    SK4D_ONLY_VULKAN(
